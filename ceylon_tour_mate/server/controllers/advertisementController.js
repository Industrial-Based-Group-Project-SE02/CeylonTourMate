...
exports.createAdvertisement = async (req, res) => {
  try {
    ...
    if (displayOrder !== undefined && displayOrder !== null) {
      if (displayOrder < 0) {
        return res.status(400).json({ error: 'Display order cannot be negative' });
      }

      // Check if display order already exists
      const existingOrder = await pool.query(
        'SELECT id FROM advertisements WHERE display_order = $1',
        [displayOrder]
      );

      if (existingOrder.rows.length > 0) {
        return res.status(400).json({ error: 'Display order already in use. Please choose a different order number.' });
      }
    }
...
exports.updateAdvertisement = async (req, res) => {
  try {
    ...
    if (displayOrder !== undefined && displayOrder !== null) {
      ...
      // Check if display order already exists (excluding current advertisement)
      const existingOrder = await pool.query(
        'SELECT id FROM advertisements WHERE display_order = $1 AND id != $2',
        [displayOrder, id]
      );
