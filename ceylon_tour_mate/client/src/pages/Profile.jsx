...
function Profile() {
  ...
  const handleUpdateProfile = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.put(`${API_BASE_URL}/api/auth/profile`, profile, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
  ...
  const handleUploadPicture = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await axios.post(`${API_BASE_URL}/api/auth/profile-picture`, formData, {
        headers: {
          'Authorization': `Bearer ${token}`,
      ...
      showToast('error', error.response?.data?.error || 'Failed to upload profile picture');
    } finally {
      setUploading(false);
    }
  };
  ...
  const getProfileImageUrl = (picturePath) => {
    ...
    return getImageUrl(picturePath);
  };
